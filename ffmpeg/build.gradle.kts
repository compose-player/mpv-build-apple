  ...
  createFramework = {
    doLast {
      ...
      for (framework in dependency.frameworks) {
        ...
        for (link in links) {
          val newPath = when {
            link.startsWith("@rpath/libshaderc_shared.") -> "@rpath/Shaderc_combined.framework/Shaderc_combined"
            link.startsWith("@rpath/libmbedtls.") -> "@rpath/Mbedtls.framework/Mbedtls"
